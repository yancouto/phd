...
fn assert_data<L: Lists<impl AggregatedData<Data = i32>>>(l: &mut L, u: usize, data: &[i32]) {
...
impl<L: Lists<AggSum>> LTests<L> {
    ...
    fn assert_data(l: &mut L, u: usize, data: &[i32]) {
    ...
    fn assert_same_content(l: &mut L, u: usize, v: usize) -> bool {
        if l.len(u) != l.len(v) {
            return false;
        }
        (0..l.len(u)).all(|i| {
            let u_i = l.find_kth(u, i);
            let v_i = l.find_kth(v, i);
            let n1 = l.data(u_i);
            let n2 = l.data(v_i);
            n1 == n2
        })
    ...
    fn test_concat() {
        ...
        assert_eq!(l.data(r3), &8);
        assert_eq!(l.data(r2), &3);
        assert_eq!(l.data(r0), &1);
    ...
    fn test_split() {
        let l = &mut Self::build(&[1, 2, 3, 7, 9, 2]);
    ...
    fn test_change_data() {
        let (l, r) = (&mut Self::build(&[1, 2, 4]), 0);
    ...
    fn test_find_element() {
        let l = &mut Self::build(&[0, 0, 1, 0, 3, 0, 2, 0, 1, 1000]);
        let mut idx_of_kth_value = |mut k: i32, expected: Idx| {
            let v = l.find_element(0, move |s: SearchData<'_, AggSum>| {
                dbg!(k, &s);
                dbg!(if s.left_agg.0 >= k {
